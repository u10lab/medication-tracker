...
### Phase 1: 環境構築・認証基盤
- [ ] **バックエンド環境構築**
  - [ ] Laravel 11 APIプロジェクト作成（別ディレクトリ）
  - [ ] Supabaseプロジェクト作成・設定（Auth + PostgreSQL DB）
  - [ ] Laravel環境変数設定(.env) - Supabase接続情報
  - [ ] Railway初期デプロイ設定（Laravel API用）
- [ ] **フロントエンド拡張**（既存Vue.jsプロジェクトに追加）
  - [x] Vue.js 3 + Vite + TailwindCSS（既存）
  - [ ] Pinia状態管理ライブラリ追加
  - [ ] Axios HTTP クライアント追加
  - [ ] Supabase JavaScript SDK追加
  - [ ] 認証状態管理ストア作成
- [ ] **認証システム統合**
  - [ ] フロントエンドにSupabase Auth統合
...
```
medication-tracker/           # フロントエンド（既存Vue.jsプロジェクト）
├── src/
│   ├── components/          # Vueコンポーネント
│   ├── views/              # ページレベルコンポーネント  
│   ├── stores/             # Piniaストア（追加予定）
│   ├── services/           # API通信（追加予定）
│   └── router/             # Vue Router設定
├── package.json            # フロントエンド依存関係
└── ...

medication-tracker-api/      # バックエンド（新規Laravel API）
├── app/
